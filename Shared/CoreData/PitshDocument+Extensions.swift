...
//            moc.undoManager?.enableUndoRegistration()
    ...
    func performAutocorrelation(floatData: [Float], sampleRate: Double, completionHandler: @escaping (Error?) -> ()) {
        ...
        moc.perform {[weak self] in
            ...
            weakSelf.audioSampleRate = sampleRate
            weakSelf.keyPair = (root: bestKey.root, major: bestKey.major)
            weakSelf.stepSize = Int16(stepSize)
            weakSelf.frequencies = frequencies
            weakSelf.pitches = pitches
            weakSelf.normalisedPowers = normalisedPowers
            weakSelf.powers = powers
            
            (weakSelf.relatedEvents as! Set<PitshEvent>).forEach({moc.delete($0)})
            ...
            for e in events {
                ...
                weakSelf.addToRelatedEvents(PitshEvent)
              }
            
            let pitches = events.map({$0.avPitch})
            if let minPitch = pitches.min(), let maxPitch = pitches.max() {
                weakSelf.minimumVisiblePitch = minPitch - 12
                weakSelf.maximumVisiblePitch = maxPitch + 12
            }
