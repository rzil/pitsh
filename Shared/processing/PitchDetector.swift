...
class PitchDetector {
    ...
    init(audioStride: UInt32 = pitchAudioStride,
        ...
        self.A = COMPLEX_SPLIT(realp: &Areal, imagp: &Aimag)
        self.fftSetup = vDSP_create_fftsetup(vDSP_Length(log2n), FFTRadix(FFT_RADIX2))
    ...
    private func autocorrelation(audio: inout [Float]) {
        ...
        vDSP_ctoz(UnsafeRawPointer(audio).bindMemory(to: DSPComplex.self, capacity: Int(nOver2)), 2, &A, 1, vDSP_Length(nOver2))
        ...
        vDSP_ztoc(&A, 1, UnsafeMutableRawPointer(mutating: audio).bindMemory(to: DSPComplex.self, capacity: Int(nOver2)), 2, vDSP_Length(nOver2))
